...
export async function POST(
  ...
  try {
    ...
    if (updateError) {
      return NextResponse.json({ ok: false, error: updateError.message || 'Failed to approve execution' }, { status: 500 });
    }
    if (!execution?.id) {
    ...
    console.error('[admin-charity-execution-approve] POST error:', error);
